...
const APPLY_OPTIONS = { ...PLAN_ID, apply: BOOLEAN, ...PATCH_FLAGS };
...
const TEMPLATE_TOOL_SCHEMAS: Record<string, Record<string, unknown>> = {
  ...
  instantiate_template: objectSchema({ name: STRING, ...APPLY_OPTIONS }, ["name"]),
  create_http_api_plan: objectSchema(APPLY_OPTIONS),
  create_login_flow: objectSchema(APPLY_OPTIONS),
  create_bearer_token_flow: objectSchema(APPLY_OPTIONS),
  create_crud_flow: objectSchema(APPLY_OPTIONS),
  create_csv_driven_flow: objectSchema(APPLY_OPTIONS),
