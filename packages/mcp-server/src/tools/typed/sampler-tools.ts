import { SAMPLER_TOOL_INPUT_SCHEMAS } from "../input-schemas.js";
...
export function registerSamplerTools(registry: ToolRegistry): void {
  for (const name of SAMPLER_TOOLS) {
    registry.register({
      name,
      description: `Typed sampler tool: ${name}`,
      inputSchema: SAMPLER_TOOL_INPUT_SCHEMAS[name] ?? { type: "object", additionalProperties: true }
    });
  }
}
