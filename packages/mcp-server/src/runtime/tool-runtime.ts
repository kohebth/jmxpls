...
export class JmxplsRuntime {
  ...
  private getPlanLanguage(input: ToolCallInput): ToolCallResult { return this.withSession(input, (session) => { const mode = (optionalString(input, "mode") ?? "outline") as PlanLanguageMode; const detail = optionalString(input, "detail") as PlanLanguageDetail | undefined; const redaction = optionalString(input, "redaction") as RedactionMode | undefined; const subtreeNodeId = optionalString(input, "subtreeNodeId") ?? optionalString(input, "nodeId"); const format = optionalString(input, "format") ?? "object"; const document = projectPlanLanguage(session.semanticPlan(), { mode, ...(detail ? { detail } : {}), ...(redaction ? { redaction } : {}), ...(subtreeNodeId ? { subtreeNodeId } : {}) }); return format === "json" || format === "yaml" ? serializePlanLanguage(document, format) : document; }); }
...
function semanticPatchFromInput(input: ToolCallInput): SemanticPatch { const patch = input.patch as SemanticPatch | undefined; if (patch && Array.isArray(patch.operations)) return { ...patch, validate: patch.validate ?? (typeof input.validate === "boolean" ? input.validate : true), ...(patch.dryRun === undefined && typeof input.dryRun === "boolean" ? { dryRun: input.dryRun } : {}) }; if (Array.isArray(input.operations)) return patchWithFlags(input, input.operations as SemanticPatchOperation[]); throw new Error("patch.operations is required"); }
function patchWithFlags(input: ToolCallInput, operations: SemanticPatchOperation[]): SemanticPatch { return { operations, validate: typeof input.validate === "boolean" ? input.validate : true, ...(typeof input.dryRun === "boolean" ? { dryRun: input.dryRun } : {}) }; }
