...
JMX does not carry stable node IDs. On save, `jmxpls` writes `<plan>.jmxpls.meta.json` next to the plan, for example `load.jmx` produces `load.jmxpls.meta.json`, with current stable node IDs, JMX paths, fingerprints, and names. On open, matching sidecar fingerprints are reapplied to the canonical tree so semantic resources, diffs, and patches can keep stable IDs across reloads while the `.jmx` remains valid without the sidecar. If a node changed enough to alter its fingerprint, reconciliation falls back to the same JMX path to repair stale sidecar mappings conservatively.
