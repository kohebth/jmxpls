...
JMeter-backed validation depends on the Java bridge and the JMeter/plugin classpath available to that bridge. Configure `JMXPLS_JAVA_BRIDGE_JAR` before treating `validate_with_jmeter` as authoritative for plugin-heavy plans. If bridge diagnostics include class-loading failures, MCP responses add `JMX_JMETER_PLUGIN_CLASS_MISSING` with guidance to install the missing plugin jars on the bridge classpath.
